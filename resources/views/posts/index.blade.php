<x-app-layout>
    <div class="container py-8">
        <div class="flex flex-col justify-center h-44 mb-8 lg:h-96  w-full rounded-md "
            style="background-image: url(https://media-cdn.tripadvisor.com/media/photo-m/1280/1a/e4/f3/d8/interior-porfirios-puebla.jpg)">
        ...
        <div class="flex flex-col justify-center mb-4 items-center">
            <a href="{{ route('reservations.create') }}" class="text-blue-800  text-3xl uppercase leading-loose text-center">
                ¿Aún no tienes tu reservación? ¡Creala ahóra!
            </a>
            <a href="{{ route('reservations.index') }}"
                class="text-blue-800 text-base uppercase leading-loose mb-2 text-center">¿Ya
                tienes una? ¡Revísala aquí!</a>
            @if (session('info'))
                <div
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 ">
            @foreach ($posts as $post)
                <article class="shadow-lg rounded-md w-full h-80 bg-cover bg-center" style="background-image: url(@if($post->image){{Storage::url($post->image->url)}} @else https://image.freepik.com/vector-gratis/personas-que-buscan-carpeta-pagina-destino_52683-23928.jpg @endif)">
